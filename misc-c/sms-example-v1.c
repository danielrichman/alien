...
int main(int argc, char **argv)
{
  ...
  int i, l, m, n;  /* Assorted uses and temporary values */
  ...
  for (i = 0; i < intext_length; i++)
  {
    ...
    for (n = 0; n < 7; n++)                /* For each of the 7 bits... */
    {
      ...
      {
        sms_string[m] |= l;                   /* Set the bit by ORing l */
      }

      /* Now shift l left, effectivly multiplying by 2 and selecting the next
       * bit in the output string */
      l = l << 1;

      /* If the 9th bit is set, ie. it == 256, then we must roll it back to 
       * 1 (1st bit) and move onto the next byte. */
      if (l == 256)
      {
        l = 1;
        m++;
